...
경계를 건너는 건 전부 JSON 문자열입니다. webview-ts는 그 문자열을 기준으로 양쪽에서 type-safe하게 관리할 수 있게 합니다.
...
| 프레임워크 (`react`, `vue`, `react-native`) | core를 감싼 얇은 래퍼. 해당 프레임워크가 맡을 수 있는 역할을 전부 export (웹 = 클라이언트 **그리고** 호스트) | lint 경계 룰                  |
| `devtools`, `cli`                           | 사이드카 — 정해진 관찰 지점(seam)으로만 관찰, `shared`만 안다                                                | lint 경계 룰                  |
...
`ActionStateManager`(`shared` 소속)는 액션 하나를 담당하는, 프레임워크와 무관한 비동기 상태 기계입니다. `status`, `data`, `error`, `isLoading`을 풀 모델(`subscribe`/`getSnapshot` — React `useSyncExternalStore`용)과 푸시 모델(`watch` — Vue/Svelte/Solid용) 두 가지로 노출합니다. React·Vue 패키지는 이 위의 얇은 래퍼일 뿐이라, 새 프레임워크를 지원하는 일은 재작성이 아니라 구독 파일 하나를 추가하는 일입니다.

캐시는 `ActionCache`가 담당합니다. `BridgeClient`가 액션당 하나씩 소유하고, 그 액션을 쓰는 모든 컴포넌트가 나눠 씁니다 — [Timeout, Retry & Cache](../guides/timeout-retry-cache)를 참고하세요.
